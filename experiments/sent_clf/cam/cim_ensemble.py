...
parser.add_argument('-base', '--base', type=str, help='Options: base|tapt', default='basil_tapt')
    ...
    for v in range(len(fold['train'])):
        ...
        if EMB_TYPE not in ['use', 'sbert']:
            ...
            if BASE == 'basil_tapt':
                ...
                s = 34
            embed_fp = f"data/embeddings/rob_{BASE}/rob_{BASE}_sequential_{s}_bs16_lr1e-05_f{fold['name']}_basil_w_{EMB_TYPE}"
            weights_matrix = get_weights_matrix(data, embed_fp, emb_dim=EMB_DIM)
            logger.info(f" --> Loaded from {embed_fp}, shape: {weights_matrix.shape}")
