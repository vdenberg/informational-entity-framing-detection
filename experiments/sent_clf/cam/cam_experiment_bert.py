...
#                    PREPROCESS DATA
# =====================================================================================
empty_sents = ['11fox23', '13fox8', '14fox8', '14hpo15']
...
folds = spl.apply_split(
    features=['story', 'source', 'id_num', 'context_doc_num', 'token_ids', 'token_mask', 'position'])
if DEBUG:
                ...
                if os.path.exists(setting_table_fp):
                    ...
                    for fold in folds:
                        ...
                        if os.path.exists(fold_table_fp):
                            ...
                            val_results = {'model': model_type, 'fold': fold["name"], 'seed': SEED_VAL,
                                           'bs': BATCH_SIZE, 'lr': LR, 'h': HIDDEN, 'set_type': 'dev'}
                            test_results = {'model': model_type, 'fold': fold["name"], 'seed': SEED_VAL,
                                            'bs': BATCH_SIZE, 'lr': LR, 'h': HIDDEN, 'set_type': 'test'}

                            cam = ContextAwareClassifier(start_epoch=START_EPOCH, cp_dir=CHECKPOINT_DIR,
                                                         tr_labs=fold['train'].label,
                                                         weights_mat=fold['weights_matrix'], emb_dim=EMB_DIM,
                                                         hid_size=HIDDEN, layers=BILSTM_LAYERS,
                                                         b_size=BATCH_SIZE, lr=LR, step=1, gamma=GAMMA,
                                                         context_naive=CN)

                            cam_cl = Classifier(model=cam, logger=logger, fig_dir=FIG_DIR, name=fold_name,
                                                patience=PATIENCE, n_eps=N_EPOCHS,
