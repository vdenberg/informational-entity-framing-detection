...
def eval(labels, preds, av_loss=None, set_type="", name=""):
    ...
    # make conf_mat
    conf_mat = f"(fn: {metrics_dict['tn']} tp: {metrics_dict['tp']} fn: {metrics_dict['fn']} fp {metrics_dict['fp']})"
    if av_loss:
        metrics_string = f"On {set_type} {conf_mat}: loss {metrics[4]} acc {metrics[0]} prec {metrics[1]} rec {metrics[2]} -> {set_type} f1: {metrics[3]}"
    else:
